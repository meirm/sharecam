 ...
 * 
 * g++ -Wall  -c  srv0.cpp
 * g++ -Wall -o "srv0" srv0.o -I /usr/include/boost  `pkg-config --libs --cflags opencv` -std=gnu++11 -lrt -pthread
 */
using namespace cv;
int main(int argc, char* argv[])
{
  ...
  while (cv::waitKey(10) <1) {
    ...
    capture_device >> captured_image;
    cv::imshow("Server window", captured_image);

    // Copy the captured image to the shared memory 
    memcpy((void *)shared_image_data_ptr, captured_image.data, data_size);
